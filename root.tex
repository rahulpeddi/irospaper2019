...
Contrary to most model based approaches, we assume no knowledge of specific dynamical parameters, and only a general idea of the dynamical model and controller architecture. 
%This makes parameter estimation a rather unfeasible and inefficient task. 
Instead of estimating parameters, we use information obtained from human-piloted training demonstrations to directly generate control inputs to track any trajectory, by way of training and analyzing a regression model and implementing online corrections.
...
\begin{figure}[h]
	\centering
	\subfigure[Sample of a Single Demonstration\label{fig:sampleintegral}]{\includegraphics[width=1\linewidth]{sampleintegral.png}}
	\subfigure[Sample Trajectory\label{fig:sampt}]{\includegraphics[width=1\linewidth]{sampletrajectory.png}}
	\subfigure[Sample Velocity\label{fig:sampv}]{\includegraphics[width=1\linewidth]{samplevelocity.png}}
...
%must be fixed to a certain length (time) and travel a certain distance.
%
...
%The low-level controller, which consists of a series of PID loops [CITE], generate the angle inputs and attitude control that follow the desired trajectory, $\bm{p_r}$. The attitude controller and motion dynamics generate the thrust and moment values applied by each rotor. Lastly, the quadrotor body dynamics give the position of the system, which is then fed back into the position controller. This control architecture assumes that the quadrotor is moving at a constant z level with a zero yaw constraint, which is discussed later on in the paper.
...
In Fig.~\ref{fig:train}, we show the raw data received from the demonstrations. In this case, we have training samples where the pilot flies forward and then stops, and these are shown in Fig.~\ref{fig:trajs}.

\begin{figure}[H]
	\centering
	\subfigure[Training Teleoperation Commands \label{fig:traincmd}]{\includegraphics[width=0.49\linewidth]{finalfigs/traincommand.png}}
	\subfigure[Training Trajectories \label{fig:traintraj}]{\includegraphics[width=0.49\linewidth]{finalfigs/trajset.png}}
...
\begin{figure}[H]
	\centering
	\subfigure[Training Intermediate Velocities \label{fig:traincmd}]{\includegraphics[width=0.49\linewidth]{finalfigs/trainelabvel.png}}
	\subfigure[Training Intermediate Trajectories \label{fig:traintraj}]{\includegraphics[width=0.49\linewidth]{finalfigs/trainelabtraj.png}}
	\caption{Training Set}
	\label{fig:trainelab}
\end{figure}
...
In our case, there are two inputs we are primarily concerned about for prediction; distance and time, as in \eqref{eq:integralfit} and \eqref{eq:ssvelfit}. The distance and time values are used together as input pairs, so we use standard error ellipses to connect the two together \cite{stdellipse}. A general equation for a standard error ellipse for our maximum departure is shown below: %\NB{this section is too long..just go to the point. State that we are using this method to compute error region or whatever they are called} %\NB{I don't understand this..why are you separating this values? because we obtain a height and a length of these ellipses}
\begin{equation}
    \bigg(\frac{x}{\sigma_{d,est}t^*}\bigg)^2 + \bigg(\frac{y}{\sigma_{t,est}t^*}\bigg)^2 = 1
\end{equation}
where $\sigma_{d,est}$ and $\sigma_{t,est}$, are standard error values for distance and time, respectively. The ellipses at each point in our training data are calculated using the following equations:
...
In this section, we discuss how we generate smooth trajectories, and then decompose the trajectories subject to the bounds from the regression analysis.
...
Fig.~\ref{fig:trajdec}(a) shows an example of a generated trajectory with its discretization while Fig.~\ref{fig:trajdec}(b) displays the $x$ and $y$ sequences together.
\begin{figure}[h]
	\centering
...
$g_u$ and $\bar{j}_u$ are obtained using equations \eqref{eq:integralfit} and \eqref{eq:ssvelfit} with each component of our trajectory, and the offline training samples are leveraged to generate a new string of commands.% We first check back with our generated trajectory components to ensure that the distances of each segment are within the bounds of our error ellipses, as calculated in \eqref{eq:bounds}, and displayed in Fig.\ref{fig:preds}. If the error regions are violated, there is a possibility that the UAV will not reach the points in the desired trajectory, which is an indication of the strength and viability of the training set \NB{so what you do? We need to state and formulate how the trajectory is adapted to avoid this problem. Perhaps we can present the trajectory generation here and discuss in detail how to sample the intermediate waypoint to avoid problems}. After checking for possible violation of the error ellipses, commands are generated for each component of the generated trajectory. %\NB{this sentence is not good. A reviewer her will think that you are not showing how to generate the commands and we already provided this in section \ref{sec:traj}...again you need to put yourself in the reviewer's shoes and proofread the paper as if you are a reviewer}.
...
\end{equation}
 The lowest error in $\bm{e} = \{e_1,\ldots,e_m\}$ then found and is paired with the appropriate pre-trained sample, $\bm{J}^*$, %\NB{show $\bm{e}$}
\begin{equation}
...
where $j$ is an entry in vector $\bm{J}^*$ and $a_i$ represents the coefficient of the best-fit cubic function at each point. %\NB{no clue what this means...what are these coefficients? coefficients of the cubic best fit to find the values that would go in between each entry}.
Bicubic interpolation takes the weighted sum of the four nearest neighbors of each entry in the command vector in order to identify a function for the intermediate points between each value in $\bm{J}^*$. This method is a result from \cite{bicfeatures}, where it is described in more detail. After resizing, we obtain the time adjusted input vector $\bm{J}'$.
...
\end{equation}
Lastly, we check for integral error. While using the fitted average input, $\bar{j}_u$, scales the commands, there may still be some error between the integral of our new command string and the desired integral, $g_u$. The ratio between the two integrals is leveraged to re-scale the command string:
\begin{equation} \label{eq:iintscale}
\bm{J}_a = \bm{J}'\bigg(\frac{g_u}{\int_0^{t_u}\bm{J}''dt}\bigg)
\end{equation}


In Fig.~\ref{fig:gensample}, the steps of command generation are visualized. % \NB{rewrite this sentence}.
In Fig.~\ref{fig:ogstring}, we start with the original pre-trained command string that was shown above in Fig.~\ref{fig:sampleintegral}. This flight lasted approximately $4.4$ seconds for $3$ meters. For testing purposes, our user input values are $d_u=3.5$m and $t_u=3.75$s. As indicated, a time adjustment is made first; this is shown in Fig.~\ref{fig:timeadj}. It is important to note that the average command, indicated by the dashed line, is still the same, which is a verification of feature retention aspect of bicubic interpolation from \eqref{eq:bicinter}. If the command string were stretched without using an interpolation method, we would expect slight variation in the mean, which could damage the integrity of the final steps, shown in Fig.~\ref{fig:fullyadj}, which is obtained using \eqref{eq:imgscale} and \eqref{eq:iintscale}. At this point we have obtained the autonomous command string $\bm{J}_a$, which is then sent to the UAV to follow the trajectory that has been set.
...
From the autonomous command, the expected path of the vehicle can be generated. The integral of the final command string is taken at each time-step to determine the expected position at all times. Because we know the relationship between the integral and the distance travelled at all times, we are able to invert the equation from the TPS regression for the integral, \eqref{eq:integralfit}, such that the inputs are time and integral and the output is the distance.
...
The commands generated in the previous section are generated and sent to the UAV in open-loop. While we expect open-loop results to be reasonably accurate, we desire more assured trajectory tracking by closing the loop by controlling for any possible error. While executing generated command string, $\bm{J_a}(t)$, we monitor the error between the position of the vehicle, $\bm{p}(t)$ and the reference position as per the generated trajectory, $\bm{p_r}(t)$:% \NB{how is this calculated?}:
\begin{equation}
    \xi(t) = \bm{p_r}(t)-\bm{p}(t)
\end{equation}

This error, $\xi(t)$, is attributed to the most recent command sent to the system. In order to adjust the next input to correct for the error, we use a proportional controller to adjust the subsequent command, $\bm{J}_a(t+1)$, to obtain the closed-loop command $\bm{J}_c(t+1)$ as follows:
...
where $\bar{j}_a$ is the average input from the open loop commands and $r_p$ is a ratio applied to the average input. The gain is chosen in this manner as to avoid drastic corrections that would affect the stability of the system. If the error at time $t$, $\xi(t)$, is very high, then having $r_p = 1$ could result in a drastic and unstable change in the control input. The correction ratio depends on the amount of adjustment desired by the user, which depends on certain constraints, such as flight time and flight conditions (i.e., different correction ratios may be desirable for indoor and outdoor flight). Using this controller, we are able lower the position error at every time step.  %\NB{what threshold?} 
...
The first experiments consisted of tracking a diagonal distance, to validate that $x$ and $y$ components as discussed in Section~\ref{sec:traj} can be attained accurately. Starting from the world frame origin, we set the target to $(-1.7,2.3)$, that is, $-1.7$m in the $x$ direction and $2.3$m in the $y$ direction. The time set for this trajectory was $5.25$s. Using the regression training, we first obtain the open-loop commands for the $x$ and $y$ directions.
\begin{figure}[h]
	\centering
	\subfigure[Open-Loop X Commands \label{fig:openxcmds}]{\includegraphics[width=0.48\linewidth]{images/OPENXcmds.png}}
    \subfigure[Open-Loop Y Commands \label{fig:openycmds}]{\includegraphics[width=0.48\linewidth]{images/OPENYcmds}}
	\caption{Open-Loop Commands for X and Y Components}
	\label{fig:1723}
\end{figure}

The open-loop and closed-loop versions of the diagonal trajectories are presented in Fig.~\ref{fig:1723}, where it is clear that the closed-loop trajectory tracks to the target, while the open loop trajectory falls short of the setpoint. \NB{where is the error plot?}
%\NB{What does it mean "solution tracks to the target"?}

\begin{figure}[h]
	\centering
	\subfigure[Open-Loop Trajectory \label{fig:1723open}]{\includegraphics[width=0.48\linewidth]{images/1723open.png}}
    \subfigure[Closed-Loop Trajectory \label{fig:1723close}]{\includegraphics[width=0.48\linewidth]{images/1723closed.png}}
	\caption{Diagonal Trajectories of UAV Experiments}
	\label{fig:1723}
\end{figure}

In addition to the diagonal trajectories, the $x$ and $y$ component trajectories in open and closed loop are presented in Fig.~\ref{fig:expcomponents}. The $x$ trajectories were flipped to appear positive for the purposes of visualization. \NB{why? Please put them in the right side...this doesn't help.} \NB{where are the generate inputs?} \NB{you need to explain better how from the desired trajectory we decompose into two components, generate the inputs and then the results}

%\begin{figure}[h]
%	\centering
%	\subfigure[X Component Trajectories\label{fig:expxcomp}]{\includegraphics[width=0.48\linewidth]{images/17Xtraj.png}}
%    \subfigure[Y Component Trajectories \label{fig:expycomp}]{\includegraphics[width=0.48\linewidth]{images/23Ytraj.png}}
%	\caption{Component Trajectories of UAV Experiments}
%	\label{fig:expcomponents}
%\end{figure}

Within the same experiment, the commands sent to the system in open and closed loop were also recorded \NB{rewrite this section...it's very confusing...you need to state things in the same order as in your approach. Here it reads almost as if he commands were recorded but not generated by your approach.}. These commands in Fig.~\ref{fig:expcmds}, show how the commands change when we close the loop using the approach in Section~\ref{sec:adapt}. 

\begin{figure}[h]
	\centering
	\subfigure[X Component Commands\label{fig:xcmds}]{\includegraphics[width=0.48\linewidth]{images/1723Xcmds.png}}
    \subfigure[Y Component Commands \label{fig:ycmds}]{\includegraphics[width=0.48\linewidth]{images/1723Ycmds.png}}
	\caption{Commands Generated in UAV Experiments}
	\label{fig:expcmds}
\end{figure}

In Fig.~\ref{fig:xcmds}, it is evident that there is little correction being done. This is because the commands given in the $x$ direction proved to only slightly deviate from the desired target. The commands in Fig~\ref{fig:ycmds} show a clear adjustment, as the open loop trajectory clearly does not reach the target. For this experiment, the correction ratios were set to $r_p = r_i = 1/(ft_u)$, where $t_u$ is the target time set for this trajectory, and $f$ is the rate at which autonomous commands are sent to the system \NB{why??? how did you select these commands. A reviewer here will kill the entire paper for this. You need to explain why and how you are selecting these parameters!!!}. 

To further validate the approach, a square-shaped trajectory was tested. 
\begin{figure}[ht!]
	\centering
	\subfigure[Closed Loop Square \label{fig:clsquare}]{\includegraphics[width=0.48\columnwidth]{images/closedsquare.png}}
    \subfigure[Open Loop Square \label{fig:opensquare}]{\includegraphics[width=0.48\columnwidth]{images/opensquare.png}}
