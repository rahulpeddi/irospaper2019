...
\end{itemize}
The steady-state command and integral are necessary portions of the analysis \NB{aren't we already discussing this before in section IV?}because of the directly proportional relationship between teleop commands and system velocity. The steady-state command, in this case, provides information about the pilot's average in-flight velocity, and is obtained by taking the mean of all entries in $\mathbf{x_i}$. The integral, meanwhile, describes the area under the string of commands. Because the teleop commands are proportional to velocity, this area value gives important information about the distance traveled, as position is the integral of velocity over time.
\NB{here we need to show some examples of the training set. Show a few training data sampled trajectories and what we extract from these trajectories - mention how,why we are doing that}
...
Given the general framework for performing the thin-plate spline, we develop two separate relationships for our specific application\NB{connect more with the thin plate spline i.e., in our work the first regression is applied to d and t as inputs to obtain the integral of the commands $g_i = f(d_i,t_i)$ while the second one is ...}: $g_i = f(d_i,t_i)$ and $\bar{x}_i = h(d_i,t_i)$
