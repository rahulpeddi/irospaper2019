...
\begin{equation}
    \xi(t) = \bm{p}(t)-\bm{p_r}(t)
\end{equation}

The error $\xi(t)$ is attributed to the most recent command $\bm{J}_a(t)$, and a proportional controller is used to adjust the following command $\bm{J}_a(t+1)$ to obtain the closed-loop command $\bm{J}_c(t+1)$ as follows

\begin{equation}
    \bm{J}_c(t+1) = \bm{J}_a(t+1) + k_p(t)\xi(t)
\end{equation}
...
\begin{equation}
    k_p(t+1) = k_p(t) + \frac{\xi(t)}{\bm{p_r}(t)}
\end{equation}

The quantity $\frac{\xi(t)}{\bm{p_r}(t)}$ relates the current error to the desired position within the trajectory. 
