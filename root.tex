...
For our purposes, we use the minimum jerk trajectory as a starting point. Jerk is defined as the time derivative of acceleration and is often associated rapidly changing actuator forces. [CITE] The general equation for a minimum jerk trajectory is as follows:


\begin{equation}
    \mathbf{p}*(t) = \argmin_{p(t)}\int_0^T\mathcal{L}(\dddot{p},\ddot{p},\dot{p},p,t)dt =  \argmin_{p(t)}\int_0^T
\end{equation}
