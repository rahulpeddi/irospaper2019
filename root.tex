...
\NB{Motivation here is that it is hard to generate autonomous flight because every platform is different and you need to show what's the typical iteration to generate autonomous flght with quadrotor based on model. }Unmanned Aerial Vehicles have become more widespread for both civilian and military applications in the recent years. T
...
Given the general framework for performing the thin-plate spline, we develop two separate relationships for our specific application\NB{connect more with the thing plate spline i.e., in our work the first regression is applied to d and t as inputs to obtain the integral of the commands $g_i = f(d_i,t_i)$ while the second one is ...}: $g_i = f(d_i,t_i)$ and $\bar{x}_i = h(d_i,t_i)$
