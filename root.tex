...
\end{align}
where $\theta$ is an angle from $[0,\ldots,2\pi]$, and $i = 1,\ldots,m$ reflects each of the $m$ training samples, and $\bm{d}_i$ and $\bm{t}_i$ are all of the points on the perimeter of the ellipse of training point $i$. A convex hull is then computed around the ellipses to provide a concise measure for the combination of distances and times that have low error when input into the TPS regression. This convex hull is generated using the Quickhull algorithm \cite{quickhull}, in which we take the outermost points of the set ellipses and form a boundary denoted $Conv(M)$, where $M$ represents the set of all points on the perimeters of all ellipses: $\{\bm{d}_i,\ldots,\bm{d}_m\},\{\bm{t}_i,\ldots,\bm{t}_m\}$.
