...
In human piloted trajectories, a user is controlling $u_2$, $u_3$, and $u_4$ from the system dynamics, which correspond to the desired roll, pitch, and yaw angles. $u_1$ is the thrust provided to the system, which is not directly sent by the user, but it is adjusted when the pilot is modifying roll, pitch, and yaw. As previously mentioned, we have a zero yaw constraint, so the pilot does not set a desired yaw. 
