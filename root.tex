...
\textbf{Problem 1: \textit{Demonstration-based autonomous control generation}}: A UAV has the objective to navigate a certain trajectory, $\mathbf{p}_r(t),~ t \in [0,T]$, which consists of target location goals, $\mathbf{g}$, and target times, $\tau \in [0,t]$, at which each of the goals should be reached. Given $m$ human-piloted flight demonstrations, the aim is to find a policy that enables autonomous flight to track the trajectory such that the following liveness and promptness requirements are met:
\begin{enumerate}
    \item  Liveness: The UAV should always stay within a certain distance of the pre-defined trajectory:
    \begin{equation}
    ...
    \begin{equation}
        ||\mathbf{t}(t)-\mathbf{t}_r(t)|| \leq \delta_t,~\forall t \in [0,T]
    \end{equation}
    where $\mathbf{p}_r(t)$ is the reference position at time $t$, where $T$ represents an overall time horizon for the trajectory, and $\delta$ is a threshold for allowable deviation. 
...
In the human piloted demonstrations, the user is controlling $u_2$, $u_3$, and $u_4$, which correspond to the desired roll, pitch, and yaw angles. In this work, we are interested in learning from commands send by the human pilot to the UAV. For the purpose of this work, we assume that we are able to collect all information sent from the pilot to the UAV during demonstrations. We are also able to get the world frame position $\mathbf{p}(t)$ of the UAV using a motion capture system. 
