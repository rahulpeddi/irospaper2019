...
\end{align}
where $\theta$ is an angle in $[0,2\pi]$, and $i = 1,\ldots,m$ reflects each of the $m$ training samples.


Fig.~\ref{fig:preds}, shows a pictorial representation of the standard error ellipses around each training point with its convex hull. Fig.~\ref{fig:predsur} is a modified version of Fig.~\ref{fig:integs} with the convex hull \cite{bounds} created from the ellipses on Fig.~\ref{fig:preds}.
\begin{figure}[H]
	\centering
...
\begin{figure}[h]
	...
	\label{fig:1723cmds}
\end{figure}
The open-loop commands however, do not accurately send the UAV to the target; this trial of the diagonal trajectory is presented in Fig.~\ref{fig:1723open}, where it is clear that the UAV falls short of the target point. %\NB{where is the error plot?}
...
\begin{figure}[h]
    ...
    \label{fig:1723open}
\end{figure} 
To show the effectiveness of our controller, we ran the same experiment, and have compared results obtained in open and closed loop systems. In Fig.~\ref{fig:1723controlled}, the new inputs generated by the controller are shown against the old inputs generated by our regression model, and the error associated with each of the trials in the $x$ and $y$ directions is shown.
\begin{figure}[H]
	\centering
