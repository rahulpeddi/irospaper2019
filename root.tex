...
The remainder of the paper is organized as follows: In Section \ref{sec:relatedwork}, we present some work related to ours \NB{no we need to say that we present the state of the art in the literature}. \NB{what about section 3?}In Section \ref{sec:sysdyn}, we discuss the system dynamics and controller architecture, which is then followed by an overview of our methodology in Section \ref{sec:approach}. We then discuss in detail each part of our approach with training in Section \ref{sec:train}, trajectogry generation in Section \ref{sec:traj}, autonomous control generation in Section \ref{sec:generate}, and lastly the online adaptation of controls in Section \ref{sec:adapt}\NB{no need to list all subsections just list teh main section details}. Finally, we end with a discussion about our experimental validation in Section \ref{sec:exper}, and we have our conclusions and a discussion on future work in Section \ref{sec:conc}.
...
\begin{figure}[ht]
	...
	\label{fig:samps}
\end{figure}
\NB{the axis labels are wrong...please be careful. most of the axis labels are incorrect}\NB{also the x axis of the fist figrue are up to 5 whiel the next two are up to 4.5. They need to be consistent and perfectly aligned!}
...
Because the integral by itself may not be enough to characterize a unique behavior of the UAV, as the same integral can be achieved with commands strings for different traveling distances and velocities, the average input command is also of interest since \eqref{eq:cinputs} inpacats changes in traveling velocity in \eqref{eq:quadrotor_dynamics}. By using the combination of integral and average input commands, we will present an approach to generate autonomous commands during a mission defined by traveling a multiple waypoint at specified distances and times.

%can improve precision during the process of generating autonomous commands during a mission, which consists of selecting a certain distance and time.
...
\begin{figure}[H]
	\centering
	\subfigure[Training Intermediate Velocities \label{fig:trainvel}]{\includegraphics[width=0.49\linewidth]{finalfigs/trainelabvel.png}}
	\subfigure[Training Intermediate Trajectories \label{fig:trainteltraj}]{\includegraphics[width=0.49\linewidth]{finalfigs/trainelabtraj.png}}
	\caption{Training Set}
	\label{fig:trainelab}
\end{figure}
...
The offline training data is applied to a thin-plate spline (TPS) surface regression to describe the relationship between training inputs and integral and average input command. Given two inputs $x,y$, the general form of a thin-plate spline equation is 
\begin{equation}
...
where $a_1,a_x,\text{and}~a_y$ are scalar coefficients, $w_i$ is a coefficient that corresponds to each specific trial\NB{what is this w term? it's not clear. what does it mean that is specific to each trial?}, subject to the following smoothing condition: \begin{equation}
...
\end{equation}
where $r=||(x_i,y_i)-(x,y)||$.
Given the corresponding $z_i$ \NB{What is $z$?} for each $(x_i,y_i)$ pair, we solve the following linear system to obtain the coefficients $w_i,\ldots,w_m$ and $a_1,a_x,a_y$,
...
where $K_{ij} = U(||(x_i,y_i)-(x_j,y_j)||)$, $P_i* = (1,x_i,y_i)$\NB{$P_i*$ does not appear in the equation}, $\bm{0}  \in \mathbb{R}^{3\times3}$ is a matrix of zeros, $\bm{o} \in \mathbb{R}^{3\times1}$ is a column vector of zeros, $\bm{w} \in \mathbb{R}^{m\times1}$ and $\bm{z} \in \mathbb{R}^{m\times1}$ are formed from $w_i$ and $z_i$, respectively, and $\bm{a}$ is the column vector with elements $a_1,a_x,a_y$.
...
In this section, we discuss how we generate, discretize, and decompose smooth trajectories, subject to the regression analysis of the training data.
Relating back to the system dynamics, we have established that input commands are responsible for specific motions of the system. Roll enables lateral motion while pitch enables longitudinal motion. Therefore, any planar motion assuming constant $z$ and yaw \NB{symbol here} can be represented as a combination of roll and pitch commands, or commands that move the vehicle along the global $x$ and $y$ coordinates. Hence when considering a planar $x,y$ trajectory we perform a decomposition in the $x$ and $y$ directions and generate independent roll and pitch commands to follow simultaneously trajectories in the $x$ and $y$ directions, respectively. 
...
where $j$ is an entry in vector $\bm{J}^*$ and $a_i$ \NB{you have already used $a$...change this term} represents the coefficients of the best-fit cubic function at each point. %\NB{no clue what this means...what are these coefficients? coefficients of the cubic best fit to find the values that would go in between each entry}.
...
From the autonomous command, the expected path of the vehicle is generated. The integral of the final command string is taken at each time-step to determine the expected position at all times \NB{how? not clear...elaborate}. Because we know the relationship between the integral and the distance travelled at all times, we are able to invert the equation from the TPS regression for the integral, \eqref{eq:integralfit}, such that the inputs are time and integral and the output is the distance travelled \NB{this sentence doesn't make sense}:
...
\end{equation}
%\NB{mention why you are doing this that is because in this way the corrective action is not too high and tailored based on the command that on average it should receive and hus do not overshoot}
...
\begin{figure}[ht]
    ...
    \subfigure[X Commands \label{fig:xcmds}]{\includegraphics[width=0.9\linewidth]{images/CLOSEDXcmds.png}}
    \subfigure[X Trajectories in Open and Closed Loop \label{fig:x2traj}]{\includegraphics[width=0.9\linewidth]{images/Xtrajs.png}}
    ...
    \subfigure[Y Trajectories in Open and Closed Loop \label{fig:y2traj}]{\includegraphics[width=0.9\linewidth]{images/Ytrajs.png}}
