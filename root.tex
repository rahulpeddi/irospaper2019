...
The parameters shown, however, are difficult to identify given a different UAV, and the controller architecture shown in Fig.\ref{fig:ctrl} consists of multiple PID loops, and therefore multiple parameters that require platform-specific tuning for autonomous flight and control. It is this requirement that calls for a different method that can be used to enable autonomous flight.

\subsection{Learning by Demonstration}

In human piloted trajectories, a user is controlling $u_2$, $u_3$, and $u_4$ from the system dynamics, which correspond to the desired roll, pitch, and yaw angles. $u_1$ is the thrust provided to the system, which is not directly sent by the user, but it is adjusted when the pilot is modifying roll, pitch, and yaw. In our case, we have a zero-yaw constraint for ease of discussion.
...
This data, however, would only be effective if trajectories involved stopping at each waypoint. In order to further expand the information obtained, considerations were made on the training data to include trajectories for demonstrations where the velocity does not start and end at zero.
...
Having trained on not only start-to-stop trajectories, but also on segments enables more specific command generation based on where the system is in relation to the provided trajectory. For example, if the command for the middle the trajectory (i.e., between two points that neither the start nor end) needs to be generated, then the training from the trajectories in Fig.~\ref{fig:trainelab} is used.
...
The quantity $\frac{\xi(t)}{\mathbf{p_r}(t)}$. 
