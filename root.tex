...
\end{equation}
 The lowest error is then found and is paired with the appropriate pre-trained sample, $\bm{J}^*$, \NB{show $\bm{e}$}
\begin{equation}
...
In Fig.~\ref{fig:gensample}, we show visually the steps of command generation \NB{rewrite this sentence}. In Fig.~\ref{fig:ogstring}, we start with the original pre-trained command string that was shown above in Fig.~\ref{fig:sampleintegral}. This flight travelled \NB{lasted?}approximately $4.4$ seconds for $3$ meters. For testing purposes, our use input values are $d_u=3.5$m and $t_u=3.75$s. As indicated, a time adjustment is made first; this is shown in Fig.~\ref{fig:timeadj}. It is important to note that the average command, indicated by the dashed line, is still the same, which is a verification of feature retention aspect of bicubic interpolation from \eqref{eq:bicinter}. If the vector were stretched without using an interpolation method, we would expect slight variation in the mean, which could damage the integrity of the final step, shown in Fig.~\ref{fig:fullyadj}, which is obtained using \eqref{eq:imgscale}. At this point we have obtained the autonomous command string $\bm{J}_a$, which is then sent to the UAV to follow the trajectory $\bm{p_r}(t)$.
...
where $\bar{j}_a$ is the average input from the open loop commands, and $\bm{J}_a(t)$ is the previously sent input command \NB{is this a scalar, matrix, vector? bold upper case letters usually indicate a matrix usually or a vector...we need to mention that it is the component at time t}. In \eqref{eq:ctrls}, $r_p$ and $r_i$ represent the ratios applied to each of the gains. These correction ratios depend on the amount of adjustment desired by the user, which depends on certain constraints, such as flight time and flight conditions \NB{I don't understand how r is chosen} (i.e., different correction ratios may be desirable for indoor and outdoor flight due to disturbances and other effects). Using this controller, we are able to ensure that the position error stays within the assigned threshold $\delta_d$, from Section~\ref{sec:prob}.  %\NB{what threshold?} 
