...
\end{align}
where $r$ is the parameter from $[0,2\pi]$, and $i = 1,\ldots,m$ and reflects each of the $m$ training samples. Fig.~\ref{fig:preds}, contains a pictorial representation of the standard error ellipses around each point. In Fig.~\ref{fig:predsur}, the surface shown in Fig.~\ref{fig:integs} is modified with a conforming boundary \cite{bounds} created by the ellipses. \{i need to explain that we don't actually care about this boundary but this is the best way to show it on the surface. also should i discuss that rectangles will have corners that poke out of the surface?\}
\begin{figure}[ht]
	\centering
