...
The offline training data is applied to a thin-plate spline (TPS) surface regression to describe the relationship between training inputs and integral and average input command. Given two inputs $\mathbf{x},\mathbf{y}$, and the output $\mathbf{z}$ ($\mathbf{x}$, $\mathbf{y}$, and $\mathbf{z}$ here are axis notations), the general form of a thin-plate spline equation is 
\begin{equation}
