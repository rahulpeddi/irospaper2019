...
In order for the system to autonomously reach a point within trajectory $\mathbf{p_r}(t)$, $g_u$ and $\bar{j}_u$ are obtained using equations \eqref{eq:integralfit} and \eqref{eq:ssvelfit}, and the offline training samples are leveraged to generate a new string of commands. A string of commands is generated for each component of a trajectory, as described in Section \ref{sec:traj}.
...
The proportional gain, $k_p(t)$, is automatically tuned based using a gradient descent method where the proportional gain is adjusted until the position error, $\xi(t)$ converges to $0$. In order to set the interval of gain increase each iteration, we use
...
The platform used for this work is the Parrot Bebop 2 Drone. The Parrot is a quadrotor that is already tuned off-the-shelf. 
