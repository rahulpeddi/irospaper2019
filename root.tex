...
In order for the system to autonomously reach a point within trajectory \NB{what does it mean to reach a point within trajectory. This is super confusing.Are you trying to reach a point within the trajectory? I thought we are trying to track a trajectory from the beginning to the end(?) This is what a reviewer will ask here. Please be more precise and refer to the waypoint generated by discretizing the trajectory} $\bm{p_r}(t)$, $g_u$ and $\bar{j}_u$ are obtained using equations \eqref{eq:integralfit} and \eqref{eq:ssvelfit}, and the offline training samples are leveraged to generate a new string of commands. We first check back with our generated trajectory components to ensure that the distances of each segment are within the bounds of our error ellipses, as calculated in \eqref{eq:bounds}, and displayed in Fig.\ref{fig:preds}. If the error regions are violated, there is a possibility that the UAV will not reach the points in the desired trajectory, which is an indication of the strength and viability of the training set \NB{so what you do? We need to state and formulate how the trajectory is adapted to avoid this problem. Perhaps we can present the trajectory generation here and discuss in detail how to sample the intermediate waypoint to avoid problems}. After checking for possible error \NB{what errors?}, commands are generated for each component of a trajectory, as described in Section \ref{sec:traj} \NB{this sentence is not good. A reviewer her will think that you are not showing how to generate the commands and we already provided this in section \ref{sec:traj}...again you need to put yourself in the reviewer's shoes and proofread the paper as if you are a reviewer}.
